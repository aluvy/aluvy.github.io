---
title: "[Deep Dive] 10장 객체 리터럴"
date: 2022-04-25 21:57:00 +0900
...
원시 타입은 단 하나의 값만 나타내지만 객체 타입은 다양한 타입의 값 (원시 값 또는 다른 객체)을 하나의 단위로 구성한 복잡한 자료구조다.   
...
이처럼 객체의 상태를 나타내는 값(프로퍼티)과 프로퍼티를 참조하고 조작할 수 있는 동작(메서드)을 모두 포함할 수 있기 때문에 상태와 동작을 하나의 단위로 구조화할 수 있어 유용하다.
...
객체의 집합으로 프로그램을 표현하려는 프로그래밍 패러다임을 객체지향 프로그램이라 한다.
...
객체 리터럴의 중괄호 { } 는 코드 블록을 의미하지 않는다는 데 주의하자.   
...
식별자 네이밍 규칙을 따르는 프로퍼티 키는 따옴표를 생략할 수 있다. (심벌 값도 프로퍼티 키로 사용할 수 있지만 문자열로 묶어야 한다.)
...
빈 문자열을 프로퍼티 키로 사용해도 에러가 발생하지는 않는다. 하지만 키로서의 의미를 가지 못하므로 권장하지 않는다.
...
대괄호 표기법을 사용해야 하는 경우 **대괄호 프로퍼티 접근 연산자 내부에 지정하는 프로퍼티 키는 반드시 따옴표로 감싼 문자열이어야 한다.**   
...
위 예제에서 ReferenceError가 발생한 이유는 대괄호 연선자 내의 때옴포로 감싸지 않은 이름. 즉 식별자 name을 평가하기 위해 선언된 name을 찾았지만 찾지 못했기 때문이다.   
...
// address프로퍼티가 존재하지 않는다. 하지만 에러가 발생하지 않는다.
...
ES6에서는 더욱 간편하고 표현력 있는 객체 리터럴 확장 기능을 제공한다.


### 10.9.1 프로퍼티 축약 표현
객체 리터럴의 프로퍼티는 프로퍼티 키와 프로퍼티 값으로 구성된다. 프로퍼티 값은 변수에 할당된 값, 즉 식별자 표식일 수도 있다.
