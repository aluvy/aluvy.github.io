...
- [The A11Y Project의 체크리스트](https://www.a11yproject.com/checklist/)

### WAI-ARIA
[Web Accessibility Initiative - Accessible Rich Internet Applications](https://www.w3.org/WAI/standards-guidelines/aria/) 문서에는 접근성을 갖춘 JavaScript 위젯을 만드는 데 필요한 기술들이 담겨있습니다.
...
- [MDN HTML 엘리먼트 참고](https://developer.mozilla.org/ko/docs/Web/HTML/Reference/Elements)

가끔 React로 구성한 코드가 돌아가게 만들기 위해 `<div>`와 같은 엘리먼트를 사용해 HTML의 의미를 깨트리곤 합니다. 특히, 목록(`<ol>`, `<ul>`, `<dl>`)과 HTML `<table>`을 사용할 때 문제가 두드러집니다. 이 경우에는, React Fragment를 사용하여 여러 엘리먼트를 하나로 묶어주는 것을 권장합니다.
...
- [The Paciello Group이 설명한 접근 가능한 이름들](https://www.tpgi.com/what-is-an-accessible-name/)
...
![alt text](/assets/images/posts/javascript/react/2023/05/31/a11y-01.png)
_https://ko.legacy.reactjs.org/docs/accessibility.html_
...
MDN Web Docs에서 [키보드로 탐색이 가능한 JavaScript 위젯](https://developer.mozilla.org/en-US/docs/Web/Accessibility/Guides/Keyboard-navigable_JavaScript_widgets)을 만드는 방법에 관해 설명한 글이 있습니다.
...
![alt text](/assets/images/posts/javascript/react/2023/05/31/a11y-02.gif)
_https://ko.legacy.reactjs.org/docs/accessibility.html_
...
![alt text](/assets/images/posts/javascript/react/2023/05/31/a11y-03.gif)
_https://ko.legacy.reactjs.org/docs/accessibility.html_
...
이 코드는 포인터 장치 사용자와 키보드 사용자 모두에게 기능을 제공합니다. 동시에 스크린 리더 사용자들을 지원하기 위해 aria-* props를 추가했습니다. 단순함을 위해 방향키로 조작하는 기능은 구현하지 않았습니다.

![alt text](//assets/images/posts/javascript/react/2023/05/31/a11y-04.gif)
_https://ko.legacy.reactjs.org/docs/accessibility.html_
...
- [The Paciello Group - 색 채도 분석기](https://vispero.com/color-contrast-checker/)
...
ESLint 플러그인인 [eslint-plugin-jsx-a11y](https://github.com/infofarmer/eslint-plugin-jsx-a11y)는 JSX 내의 접근성 문제에 대해 즉각적인 AST 린팅 피드백을 제공합니다. 많은 IDE가 코드 분석과 소스 코드 창에 이런 결과를 통합할 수 있도록 해줍니다.

[Create React App](https://github.com/facebook/create-react-app)에서는 해당 플러그인의 일부 규칙들이 활성화되어 있습니다. 더 많은 접근성 기능을 활성화하려면, 프로젝트 최상위에 아래와 같이 `.eslintrc` 파일을 생성합니다.
...
[The Accessibility](https://www.deque.com/axe/) Engine 또는 aXe는 aXe-core 기반의, 접근성 검사를 위한 브라우저 확장기능입니다.
...
#### 접근성 검사기와 접근성 트리
[접근성 트리](https://www.tpgi.com/the-browser-accessibility-tree/)는 스크린 리더와 같은 보조과학기술에 노출되어야 하는 DOM 엘리먼트에 접근 가능한 객체가 담긴 DOM 트리의 하위 집합입니다.

일부 브라우저에서는 접근성 트리 안의 각 엘리먼트의 접근성 정보를 손쉽게 확인할 수 있습니다.

- [Firefox에서 접근성 검사기를 사용하는 방법](https://firefox-source-docs.mozilla.org/devtools-user/accessibility_inspector/index.html)
- [Chrome에서 접근성 검사기를 사용하는 방법](https://developer.chrome.com/docs/devtools?hl=ko#pane)
- [OS X Safari에서 접근성 검사기를 사용하는 방법](https://developer.apple.com/documentation/accessibility/accessibility-inspector)
...
[Job Access With Speech](https://vispero.com/jaws-screen-reader-software/) 또는 JAWS는 윈도우에서 주로 쓰이는 스크린 리더입니다.
...
- [접근성](https://ko.legacy.reactjs.org/docs/accessibility.html)
